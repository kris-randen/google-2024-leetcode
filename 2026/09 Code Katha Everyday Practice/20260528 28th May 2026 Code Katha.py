...
def build_from_pre_in(preorder: List[int], inorder: List[int]) -> \
    Optional[TreeNode]:
...
def build_from_in_post(inorder: List[int], postorder: List[int]) -> \
    Optional[TreeNode]:
    inorder_pos = {val: ind for ind, val in enumerate(inorder)}
    ind = len(postorder) - 1
...
def build_from_pre_post(preorder: List[int], postorder: List[int]) -> \
    Optional[TreeNode]:
