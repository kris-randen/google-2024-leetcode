...
Stack = tuple[Optional[TreeNode], bool]
...
def lca_bst_non_guarateed(root: Optional[TreeNode], a: int, b: int) -> Optional[TreeNode]:
    lo, hi = min(a, b), max(a, b)
    candidate = lca_bst(root, lo, hi)
    return \
    (
        candidate if
        contains(candidate, lo) and
        contains(candidate, hi) else
        None
    )
