...
class TreeNode:
    def __init__(
            ...
            parent: Optional[TreeNode] = None # only for successor and predecessor
    ):
        self.val = val
        self.left = left
        self.right = right
...
def predecessor(root: Optional[TreeNode], key: int) -> Optional[TreeNode]:
    if not root:
        return None
    ...
    if key <= root.val:
        ...
        return predecessor(root.right, key) or root


def successor(root: Optional[TreeNode], key: int) -> Optional[TreeNode]:
    if not root:
        return None
    ...
    if key >= root.val:
        ...
        return successor(root.left, key) or root


def pred(p: Optional[TreeNode]) -> Optional[TreeNode]:
    if not p:
        return None
...
def succ(p: Optional[TreeNode]) -> Optional[TreeNode]:
    if not p:
        return None
...
def lca_bst_non_guaranteed(root: Optional[TreeNode], a: int, b: int) -> Optional[TreeNode]:
    candidate = lca_bst(root, a, b)
    return (
        candidate if
        contains(candidate, a) and
        contains(candidate, b) else
        None
    )
